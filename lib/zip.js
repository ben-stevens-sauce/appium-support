...
async function extractAllTo (zipFilePath, destDir, opts = {}) {
  ...
  let unzipDone = false;
  if (opts.useSystemUnzip && process.platform !== 'win32') {
    try {
      await execSystemUnzip(zipFilePath, dir);
      unzipDone = true;
    } catch (err) {
      log.warn('unzip failed; falling back to JS: %s', err);
    }
  }
  if (!unzipDone) {
 ...
 * @param {string} destDir The full path to the destination folder
 */
async function execSystemUnzip (zipFilePath, destDir) {
  const unzipPath = await assertSystemUnzip();
  // -q suppresses STDOUT
  // -o means overwrite
  // -d is the dest dir
  await exec(unzipPath, ['-q', '-o', zipFilePath, '-d', destDir]);
}
...
const assertSystemUnzip = _.memoize(
   ...
   */
  async function locateSystemUnzip () {
    try {
      const unzipPath = await fs.which('unzip');
      log.debug('found unzip at %s', unzipPath);
      return unzipPath;
    } catch {
